...
<!-- 主体内容  -->
<div class="content" >
<div class="title"> 模块授权  [ <a href="/guoji/vshop/ADMIN2/index.php/Role">返 回</a> ]</div>
<!--  功能组区域  -->
<script language="JavaScript">

function saveAccess(){
  sendForm('form1','/guoji/vshop/ADMIN2/index.php/Role/setModule/',refresh(1500));
}
...
<tr><Th class="tCenter"><a href="/guoji/vshop/ADMIN2/index.php/Role/app/groupId/<?php echo ($_GET['groupId']); ?>">应用授权</a> | 模块授权 | <a href="/guoji/vshop/ADMIN2/index.php/Role/action/groupId/<?php echo ($_GET['groupId']); ?>/appId/<?php echo ($_GET['appId']); ?>">操作授权</a>
</Th></tr>
<tr><Td class="tRight">当前组：<select id="" name="groupId" onchange="location.href = '/guoji/vshop/ADMIN2/index.php/Role/module/groupId/'+this.options[this.selectedIndex].value;" ondblclick="" class="medium" ><option value="" >选择组</option><?php  foreach($groupList as $key=>$val) { if(!empty($selectGroupId) && ($selectGroupId == $key || in_array($key,$selectGroupId))) { ?><option selected="selected" value="<?php echo $key ?>"><?php echo $val ?></option><?php }else { ?><option value="<?php echo $key ?>"><?php echo $val ?></option><?php } } ?></select>
</Td></tr>
<tr><Td class="tRight">
当前应用：<select id="" name="appId" onchange="location.href = '/guoji/vshop/ADMIN2/index.php/Role/module/groupId/<?php echo ($_GET['groupId']); ?>/appId/'+this.options[this.selectedIndex].value;" ondblclick="" class="medium" ><option value="" >选择应用</option><?php  foreach($appList as $key=>$val) { if(!empty($selectAppId) && ($selectAppId == $key || in_array($key,$selectAppId))) { ?><option selected="selected" value="<?php echo $key ?>"><?php echo $val ?></option><?php }else { ?><option value="<?php echo $key ?>"><?php echo $val ?></option><?php } } ?></select>
</Td></tr>
<tr><td>
