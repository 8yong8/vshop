...
<div class="content">
<div class="title"> 新增[ <a href="/git/vshop/admin/index.php/Position/substance/id/<?php echo ($vo["position_id"]); ?>">返回列表</a> ]</div>
<form method='post'  id="form1" action='/git/vshop/admin/index.php/Position/sub_edit?<?php echo time(); ?>' enctype="multipart/form-data">
...
<TR>
	...
	<div>
		...
		<div id="show1">
		<?php if(($vo["lit_pic"]) != ""): ?><img src="<?php echo ($vo["lit_pic"]); ?>" style="max-width:300px;max-height:500px;" class="J_preview" data-bimg="<?php echo ($vo["lit_pic"]); ?>"><?php endif; ?>
		</div>
	</div>
	</TD>
</TR>
...
<tr>
	...
	<SELECT name="data_type" id="data_type" onchange="change()">
	  <option value="product_list" <?php if(($vo["data_type"]) == "product_list"): ?>SELECTED<?php endif; ?>>产品列表</option>
	  <option value="product_detail" <?php if(($vo["data_type"]) == "product_detail"): ?>SELECTED<?php endif; ?>>产品详情</option>
	  <option value="article_list" <?php if(($vo["data_type"]) == "article_list"): ?>SELECTED<?php endif; ?>>文章列表</option>
	  <option value="article_detail" <?php if(($vo["data_type"]) == "article_detail"): ?>SELECTED<?php endif; ?>>文章详情</option>
	</SELECT>	
	</td>
</tr>

<TR class="list" <?php if(($types) == ""): ?>style="display:none"<?php endif; ?>>
	<TD class="tRight" >分类：</TD>
	<TD class="tLeft" >
	<select name="params[cat_id]" id="cat_id">
	<?php if(is_array($types)): $i = 0; $__LIST__ = $types;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$type): $mod = ($i % 2 );++$i;?><option value="<?php echo ($type[id]); ?>" <?php if(($vo[params][cat_id]) == $type[id]): ?>selected<?php endif; ?> >
...
<TR>
	...
	<TD class="tLeft" ><input type="text" class="medium bLeftRequire"  name="params[id]" value="<?php echo ($vo["params"]["id"]); ?>"></TD>
</TR>

<TR>
	<TD class="tRight" >url：</TD>
	<TD class="tLeft" >
	<input type="text" class="large bLeftRequire"  name="url" value="<?php echo ($vo["url"]); ?>">
