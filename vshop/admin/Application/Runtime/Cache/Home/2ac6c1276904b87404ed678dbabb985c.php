...
<script>
 function uponline(){
   window.location.href = "/guoji/vshop/ADMIN2/index.php/Article/uponline";
 }

 function she_uponline(){
   window.location.href = "/guoji/vshop/ADMIN2/index.php/Article/she_uponline";
 }

 function look(id){
   var url = '/guoji/vshop/ADMIN2/index.php/Article/look/id/'+id;
   window.open(url) 
 }


 function addold(){
   window.location.href = "/guoji/vshop/ADMIN2/index.php/Article/addold";
 }

 function upshowtime(id){
   var url = '/guoji/vshop/ADMIN2/index.php/Article/upshowtime/id/'+id;
   window.open(url);
 }
</script>
<!-- 内容区 -->
<div class="content">
	...
	<div class="goods mt10">
	  ...
	  <FORM METHOD=get ACTION="/guoji/vshop/ADMIN2/index.php/Article">
		<TABLE cellpadding="1" cellspacing="2" width="100%">
		<TR>
			<TD class="tRight">ID：</TD>
			<TD><INPUT TYPE="text" NAME="id" class="small" value="<?php echo ($id); ?>"></TD>
			<TD class="tRight">标题：</TD>
			<TD><INPUT TYPE="text" NAME="title" class="small" value="<?php echo ($title); ?>"></TD>
			<TD class="tRight">分类：</TD>
			<TD>	
			<select name="cid">
			<option value="" >分类选择</option>
			 <?php if(is_array($types)): $i = 0; $__LIST__ = $types;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>" <?php if(($cid) == $vo["id"]): ?>selected<?php endif; ?> >
			 <?php echo ($vo["node_name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
			</select>
			</TD>
			<TD>
			...
			<div class="login mt10" style="border: none;">
				...
				<table cellspacing="0" cellpadding="0"><tbody>
				  ...
				  <div class="datagrid-view">
					...
					<!-- Think 系统列表组件开始 -->
					<!-- My 系统列表组件开始 -->
<table id="checkList" class="list" cellpadding=0 cellspacing=0 ><tr class="row" ><th width="8"><input type="checkbox" id="check" onclick="CheckAll('checkList')"></th><th width="3%"><div class="datagrid-cell datagrid-cell-c1-name <?php if(($order) == "id"): ?>datagrid-sort-<?php echo ($sortImg); endif; ?>" style="text-align: center;cursor:pointer;color:black" onclick="javascript:sortBy('id','<?php echo ($sort); ?>','index');"><span>编号</span><span class="datagrid-sort-icon">&nbsp;</span></div></th><th width="20%">标题</th><th>分类名</th><th>添加时间</th><th>状态</th><th >操作</th></tr><?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$user): $mod = ($i % 2 );++$i;?><tr class="datagrid-row <?php if(($mod) == "1"): ?>datagrid-row-alt<?php endif; ?>" ><td><input type="checkbox" name="key"	value="<?php echo ($user["id"]); ?>"></td><td><?php echo ($user["id"]); ?></td><td style="text-align:left;height:auto;padding-left:8px;"><a href="javascript:edit('<?php echo (addslashes($user["id"])); ?>')" style="color:blue"><?php echo ($user["title"]); ?></a></td><td><?php echo ($user["cname"]); ?></td><td><?php echo (toDate($user["create_time"],'Y-m-d H#i#s')); ?></td><td><?php echo (rs($user["status"],'0#待审核#red@1#审核通过')); ?></td><td  style="text-align:left;height:auto;padding-left:8px;"> <?php echo (showStatus($user["status"],$user['id'])); ?>&nbsp;<a href="javascript:edit('<?php echo ($user["id"]); ?>')">编辑</a>&nbsp;</td></tr><?php endforeach; endif; else: echo "" ;endif; ?></table>
<!-- Think 系统列表组件结束 -->
