...
var URL = '/guoji/vshop/ADMIN2/index.php/Feedback';
...
<div class="content">
	...
	<div class="goods mt10">
	  <FORM METHOD=get ACTION="/guoji/vshop/ADMIN2/index.php/Feedback">
	  <div class="guanli">
		<TABLE cellpadding="1" cellspacing="2" width="100%">
		<TR>
			<TD class="tRight">会员名称：</TD>
			<TD><INPUT TYPE="text" NAME="member_name" class="small" value="<?php echo ($member_name); ?>"></TD>
			<TD class="tRight">内容：</TD>
			<TD><INPUT TYPE="text" NAME="content" class="small" value="<?php echo ($content); ?>"></TD>
			...
			<option value="-1" <?php if(($status) == "-1"): ?>SELECTED<?php endif; ?>>删除/驳回</option>
			</SELECT>
			</TD>
			<TD>
			...
			<div class="login mt10" style="border: none;">
				...
				<table cellspacing="0" cellpadding="0"><tbody>
				  ...
				  <div class="datagrid-view">
					<!-- My 系统列表组件开始 -->
<table id="checkList" class="list" cellpadding=0 cellspacing=0 ><tr class="row" ><th width="8"><input type="checkbox" id="check" onclick="CheckAll('checkList')"></th><th width="8%">产品编号</th><th>会员</th><th>产品信息</th><th>评价</th><th><div class="datagrid-cell datagrid-cell-c1-name <?php if(($order) == "grade"): ?>datagrid-sort-<?php echo ($sortImg); endif; ?>" style="text-align: center;cursor:pointer;color:black" onclick="javascript:sortBy('grade','<?php echo ($sort); ?>','index');"><span>星级</span><span class="datagrid-sort-icon">&nbsp;</span></div></th><th><div class="datagrid-cell datagrid-cell-c1-name <?php if(($order) == "status"): ?>datagrid-sort-<?php echo ($sortImg); endif; ?>" style="text-align: center;cursor:pointer;color:black" onclick="javascript:sortBy('status','<?php echo ($sort); ?>','index');"><span>状态</span><span class="datagrid-sort-icon">&nbsp;</span></div></th><th>评论时间</th><th >操作</th></tr><?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$user): $mod = ($i % 2 );++$i;?><tr class="datagrid-row <?php if(($mod) == "1"): ?>datagrid-row-alt<?php endif; ?>" ><td><input type="checkbox" name="key"	value="<?php echo ($user["id"]); ?>"></td><td><?php echo ($user["product_id"]); ?></td><td><?php echo ($user["member_name"]); ?></td><td><?php echo ($user["spec"]); ?></td><td style="text-align:left;height:auto;padding-left:8px;"><?php echo ($user["content"]); ?></td><td><?php echo ($user["grade"]); ?></td><td><?php echo (rs($user["status"],'0#待审核#red@1#审核通过#blue@-1#删除#black')); ?></td><td><?php echo (toDate($user["create_time"],'Y-m-d H#i#s')); ?></td><td  style="text-align:left;height:auto;padding-left:8px;"> <?php echo (showStatus($user["status"],$user['id'])); ?>&nbsp;<a href="javascript:foreverdel('<?php echo ($user["id"]); ?>')">删除</a>&nbsp;</td></tr><?php endforeach; endif; else: echo "" ;endif; ?></table>
<!-- Think 系统列表组件结束 -->
