...
<div class="content">
<div class="title">编辑数据 [ <A HREF="/guoji/vshop/admin2/index.php/Classify">返回列表</A> ]</div>
<TABLE cellpadding=3 cellspacing=3 class="add">
<FORM METHOD=POST id="form1"  action="/guoji/vshop/admin2/index.php/Classify/edit?<?php echo time(); ?>">
<TR>
	<TD class="tRight" >分类名：</TD>
	<TD class="tLeft" ><INPUT TYPE="text" class="medium bLeftRequire"  check='Require' warning="用户名不能为空" NAME="title" value="<?php echo ($vo["title"]); ?>"></TD>
</TR>
...
<TR>
	...
	<select name="pid">
	  ...
	  <?php if(is_array($types)): $i = 0; $__LIST__ = $types;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$type): $mod = ($i % 2 );++$i;?><option value="<?php echo ($type["id"]); ?>" <?php if(($type["id"]) == $vo["pid"]): ?>selected<?php endif; ?>><?php echo ($type["title"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
	</select>
	</TD>
</TR>
...
<TR>
	<TD ></TD>
	<TD><INPUT TYPE="hidden" name="id" value="<?php echo ($vo["id"]); ?>" >
