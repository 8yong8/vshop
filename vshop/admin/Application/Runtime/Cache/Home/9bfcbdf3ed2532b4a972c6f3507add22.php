...
var URL = '/guoji/vshop/admin2/index.php/Zone';
		...
		<dd>
			<div class="login mt10" style="border: none;">
				...
				<table cellspacing="0" cellpadding="0"><tbody>
				  <tr>
					<td>
					  <a href="javascript:ajax_add();" class="l-btn l-btn-small l-btn-plain" group="" id="add"><span class="l-btn-left l-btn-icon-left"><span class="l-btn-text" style="impBtn hMargin fLeft shadow">添加</span><span class="l-btn-icon icon-add">&nbsp;</span></span></a>
					</td>
				    ...
				    <td>
					  <a href="javascript:resume();" class="l-btn l-btn-small l-btn-plain" group="" id="ok"><span class="l-btn-left l-btn-icon-left"><span class="l-btn-text" style="impBtn hMargin fLeft shadow">状态启用</span><span class="l-btn-icon icon-ok">&nbsp;</span></span></a>
					</td>
					<td>
					<div class="datagrid-btn-separator"></div>
					</td>
				  ...
				  <div class="datagrid-view">
					<!-- My 系统列表组件开始 -->
<table id="checkList" class="list" cellpadding=0 cellspacing=0 ><tr class="row" ><th width="8"><input type="checkbox" id="check" onclick="CheckAll('checkList')"></th><th width="8%"><div class="datagrid-cell datagrid-cell-c1-name <?php if(($order) == "id"): ?>datagrid-sort-<?php echo ($sortImg); endif; ?>" style="text-align: center;cursor:pointer;color:black" onclick="javascript:sortBy('id','<?php echo ($sort); ?>','index');"><span>编号</span><span class="datagrid-sort-icon">&nbsp;</span></div></th><th>地区名称</th><th></th><th>状态</th><th >操作</th></tr><?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$user): $mod = ($i % 2 );++$i;?><tr class="datagrid-row <?php if(($mod) == "1"): ?>datagrid-row-alt<?php endif; ?>" ><td><input type="checkbox" name="key"	value="<?php echo ($user["id"]); ?>"></td><td><?php echo ($user["id"]); ?></td><td><a href="javascript:ajax_edit('<?php echo (addslashes($user["id"])); ?>')" style="color:blue"><?php echo ($user["name"]); ?></a></td><td><?php echo ($user["pv_names"]); ?></td><td><?php echo (getStatus($user["status"])); ?></td><td  style="text-align:left;height:auto;padding-left:8px;"><a href="javascript:ajax_edit('<?php echo ($user["id"]); ?>')">编辑</a>&nbsp; <?php echo (showStatus($user["status"],$user['id'])); ?>&nbsp;<a href="javascript:foreverdel('<?php echo ($user["id"]); ?>')">删除</a>&nbsp;</td></tr><?php endforeach; endif; else: echo "" ;endif; ?></table>
<!-- Think 系统列表组件结束 -->
