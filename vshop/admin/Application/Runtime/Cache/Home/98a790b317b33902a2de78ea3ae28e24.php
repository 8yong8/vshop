...
function getGroupUser(groupId){
  location.href='/guoji/vshop/ADMIN2/index.php/Role/userList/id/'+groupId;
}
</script>
<!-- 内容区 -->
<div class="content">
	...
	<div class="goods mt10">
	  <FORM METHOD=get ACTION="/guoji/vshop/ADMIN2/index.php/Role">
	  <div class="guanli">
			...
			<div class="login mt10" style="border: none;">
				...
				<table cellspacing="0" cellpadding="0"><tbody>
				  ...
				  <div class="datagrid-view">
					<!-- My 系统列表组件开始 -->
<table id="checkList" class="list" cellpadding=0 cellspacing=0 ><tr class="row" ><th width="8"><input type="checkbox" id="check" onclick="CheckAll('checkList')"></th><th width="10%"><div class="datagrid-cell datagrid-cell-c1-name <?php if(($order) == "id"): ?>datagrid-sort-<?php echo ($sortImg); endif; ?>" style="text-align: center;cursor:pointer;color:black" onclick="javascript:sortBy('id','<?php echo ($sort); ?>','index');"><span>编号</span><span class="datagrid-sort-icon">&nbsp;</span></div></th><th>组名</th><th>上级组</th><th>状态</th><th>描述</th><th >操作</th></tr><?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$action): $mod = ($i % 2 );++$i;?><tr class="datagrid-row <?php if(($mod) == "1"): ?>datagrid-row-alt<?php endif; ?>" ><td><input type="checkbox" name="key"	value="<?php echo ($action["id"]); ?>"></td><td><?php echo ($action["id"]); ?></td><td><a href="javascript:edit('<?php echo (addslashes($action["id"])); ?>')" style="color:blue"><?php echo ($action["name"]); ?></a></td><td><?php echo (getGroupName($action["pid"])); ?></td><td><?php echo (getStatus($action["status"])); ?></td><td><?php echo ($action["remark"]); ?></td><td  style="text-align:left;height:auto;padding-left:8px;"><a href="javascript:edit('<?php echo ($action["id"]); ?>')">编辑</a>&nbsp; <?php echo (showStatus($action["status"],$action['id'])); ?>&nbsp; <a href="javascript:app('<?php echo ($action["id"]); ?>')">授权</a>&nbsp; <a href="javascript:user('<?php echo ($action["id"]); ?>')">用户列表</a>&nbsp;</td></tr><?php endforeach; endif; else: echo "" ;endif; ?></table>
<!-- Think 系统列表组件结束 -->
