...
<div class="content">
	<div class="site">
		<?php if(isset($nodeName)): ?>[<?php echo ($nodeName); ?>]<?php endif; if(($level) == "1"): ?>应用<?php endif; if(($level) == "2"): ?>模块<?php endif; if(($level) == "3"): ?>操作<?php endif; ?>列表 <span style="font-weight:normal">[ 分组:&nbsp;<?php if(is_array($groupList)): $i = 0; $__LIST__ = $groupList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$tag): $mod = ($i % 2 );++$i;?><a href="/guoji/vshop/ADMIN2/index.php/Node/index/group_id/<?php echo ($key); ?>"><?php echo ($tag); ?></a>&nbsp;<?php endforeach; endif; else: echo "" ;endif; ?><a href="/guoji/vshop/ADMIN2/index.php/Node">所有</a> ] </span>
	</div>
	<span class="line_white"></span>
	<div class="goods mt10">
	<div class="guanli">
	  <FORM METHOD=get ACTION="/guoji/vshop/ADMIN2/index.php/Node">
			...
			<div class="login mt10" style="border: none;">
				...
				<table cellspacing="0" cellpadding="0"><tbody>
				  <tr>
				    ...
				    <td>
					  <a href="javascript:sort();" class="l-btn l-btn-small l-btn-plain" group="" id="restore"><span class="l-btn-left l-btn-icon-left"><span class="l-btn-text" style="impBtn hMargin fLeft shadow">排序</span><span class="l-btn-icon icon-restore">&nbsp;</span></span></a>
					</td>
					<td>
					<div class="datagrid-btn-separator"></div>
					</td>
				  ...
				  <div class="datagrid-view">
					<!-- My 系统列表组件开始 -->
<table id="checkList" class="list" cellpadding=0 cellspacing=0 ><tr class="row" ><th width="8"><input type="checkbox" id="check" onclick="CheckAll('checkList')"></th><th width="10%"><div class="datagrid-cell datagrid-cell-c1-name <?php if(($order) == "id"): ?>datagrid-sort-<?php echo ($sortImg); endif; ?>" style="text-align: center;cursor:pointer;color:black" onclick="javascript:sortBy('id','<?php echo ($sort); ?>','index');"><span>编号</span><span class="datagrid-sort-icon">&nbsp;</span></div></th><th><div class="datagrid-cell datagrid-cell-c1-name <?php if(($order) == "name"): ?>datagrid-sort-<?php echo ($sortImg); endif; ?>" style="text-align: center;cursor:pointer;color:black" onclick="javascript:sortBy('name','<?php echo ($sort); ?>','index');"><span>名称</span><span class="datagrid-sort-icon">&nbsp;</span></div></th><th>显示名</th><th>序号</th><th>显示</th><th>状态</th><th >操作</th></tr><?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$node): $mod = ($i % 2 );++$i;?><tr class="datagrid-row <?php if(($mod) == "1"): ?>datagrid-row-alt<?php endif; ?>" ><td><input type="checkbox" name="key"	value="<?php echo ($node["id"]); ?>"></td><td><?php echo ($node["id"]); ?></td><td style="text-align:left;height:auto;padding-left:8px;"><a href="javascript:child('<?php echo (addslashes($node["id"])); ?>')" style="color:blue"><?php echo ($node["name"]); ?></a></td><td><a href="javascript:edit('<?php echo (addslashes($node["id"])); ?>')" style="color:blue"><?php echo ($node["title"]); ?></a></td><td><?php echo ($node["sort"]); ?></td><td><?php echo (rs($node["is_display"],'1#显示#blue@0#不显示#red')); ?></td><td><?php echo (getStatus($node["status"])); ?></td><td  style="text-align:left;height:auto;padding-left:8px;"><a href="javascript:edit('<?php echo ($node["id"]); ?>')">编辑</a>&nbsp;</td></tr><?php endforeach; endif; else: echo "" ;endif; ?></table>
<!-- Think 系统列表组件结束 -->
