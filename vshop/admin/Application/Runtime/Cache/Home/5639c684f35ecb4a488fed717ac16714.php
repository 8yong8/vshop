...
var URL = '/guoji/vshop/ADMIN2/index.php/Shaidan';
...
<script>
  ...
  function ajax_edit(id){
	jQuery.ajax({
	  type:'GET',
	  url:'/guoji/vshop/ADMIN2/index.php/Shaidan/edit',
	  data:'id='+id,
	...
	/*art.dialog.open('/guoji/vshop/ADMIN2/index.php/Shaidan/edit/id/'+id,
    {title: '编辑', width: 285, height: 350});*/
  }

  function look(id){
 	$.ajax({
	  type:'GET',
	  url:'/guoji/vshop/ADMIN2/index.php/Shaidan/look',
	  data:'id='+id,
	  success:function(html){
		  art.dialog({
			//time: 1.5,
...
<div class="content">
	...
	<div class="goods mt10">
	  <FORM METHOD=get ACTION="/guoji/vshop/ADMIN2/index.php/Shaidan">
	  <div class="guanli">
		<TABLE cellpadding="1" cellspacing="2" width="100%">
		<TR>
			<TD class="tRight">会员：</TD>
			<TD><INPUT TYPE="text" NAME="member_name" class="small" value="<?php echo ($member_name); ?>" class="J_check"></TD>
			<TD class="tRight">商品ID：</TD>
			<TD><INPUT TYPE="text" NAME="shopid" class="small" value="<?php echo ($shopid); ?>"></TD>
			...
			<div class="login mt10" style="border: none;">
				...
				<table cellspacing="0" cellpadding="0"><tbody>
				  ...
				  <div class="datagrid-view">
					<!-- My 系统列表组件开始 -->
<table id="checkList" class="list" cellpadding=0 cellspacing=0 ><tr class="row" ><th width="8"><input type="checkbox" id="check" onclick="CheckAll('checkList')"></th><th width="8%"><div class="datagrid-cell datagrid-cell-c1-name <?php if(($order) == "id"): ?>datagrid-sort-<?php echo ($sortImg); endif; ?>" style="text-align: center;cursor:pointer;color:black" onclick="javascript:sortBy('id','<?php echo ($sort); ?>','index');"><span>编号</span><span class="datagrid-sort-icon">&nbsp;</span></div></th><th width="28%">商品名称</th><th>谁晒单</th><th>状态</th><th>添加时间</th><th >操作</th></tr><?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$user): $mod = ($i % 2 );++$i;?><tr class="datagrid-row <?php if(($mod) == "1"): ?>datagrid-row-alt<?php endif; ?>" ><td><input type="checkbox" name="key"	value="<?php echo ($user["id"]); ?>"></td><td><?php echo ($user["id"]); ?></td><td style="text-align:left;height:auto;padding-left:8px;"><a href="javascript:ajax_edit('<?php echo (addslashes($user["id"])); ?>')" style="color:blue"><?php echo ($user["shopname"]); ?></a></td><td><?php echo ($user["member_name"]); ?></td><td><?php echo (rs($user["status"],'0#待审核#red@1#审核通过@-1#驳回')); ?></td><td><?php echo (toDate($user["create_time"],'Y-m-d H#i#s')); ?></td><td  style="text-align:left;height:auto;padding-left:8px;"><a href="javascript:look('<?php echo ($user["id"]); ?>')">查看详情</a>&nbsp;</td></tr><?php endforeach; endif; else: echo "" ;endif; ?></table>
<!-- Think 系统列表组件结束 -->
