...
require_once("lib/md5.function.php");
require_once("lib/rsa.function.php");
class ApiCache{
	...
	//生成缓存值
	public function set($para,$value){
	  ...
	  //sign 使用POST传值
	  $value['sign'] = $sent_data['sign'];

	  $value['sign_type'] = strtoupper(trim($this->config['sign_type']));

	  unset($sent_data['sign']);

	  $sent_url = $this->gateway.CacheCreateLinkstring($sent_data);
	  //dump($value);echo $sent_url;exit;
	  $result = httpPOST($sent_url,$value);
	  //dump($result);exit;
	  return $result;
	}
